<div wire:key="{{ $todo->id }}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center">
            <input wire:click="toggle({{ $todo->id }})" type="checkbox" class="mr-2" {{ $todo->completed ? 'checked' : '' }}>

            @if($edit_to_id == $todo->id)
                <div class="flex flex-col">
                    <input wire:model="edit_todo_new_name" type="text" placeholder="Todo.."
                        class="bg-gray-100  text-gray-900 text-sm rounded block w-full p-2.5"
                        value="Todo Name">

                    @error('edit_todo_new_name')
    ...
    <span class="text-xs text-gray-500"> {{ Carbon\Carbon::parse($todo->created_at)->format('d/m/Y H:i:s') }} </span>
    <div class="mt-3 text-xs text-gray-700">
        @if($edit_to_id == $todo->id)
            <button wire:click.prevent="update({{ $todo->id }})"
